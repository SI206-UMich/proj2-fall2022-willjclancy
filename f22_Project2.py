...
.
    ...
    with open(filename, 'r') as file:
        contents = file.read()
        soup = BeautifulSoup(contents, 'html.parser')
        mydiv = soup.find('li', class_ = 'f19phm7j dir dir-ltr').text
        policy_num = mydiv[15:]
        if 'pending' in policy_num or 'Pending' in policy_num:
            policy_num = 'Pending'
        if 'not needed' in policy_num:
            policy_num = 'Exempt'
        count = 0
        bedrooms = 0
        test = soup.find_all('li', class_ = 'l7n4lsf dir dir-ltr')
        for i in test:
            if count == 1:
                bedrooms = i.text.rstrip()
            count += 1
        if 'studio' in bedrooms or 'Studio' in bedrooms:
            bedrooms = 1
        else:
        ...
        elif 'Shared' in all or 'shared' in all:
            typeroom = 'Shared Room'
        else:
            typeroom = "Entire Room"
        return (policy_num, typeroom, bedrooms)
...
def get_detailed_listing_database(html_file):
    ...
    """
    output = get_listings_from_search_results(html_file)
    result = []
    for data in output:
        new_data = get_listing_information(data[2])
        result.append((data[0], data[1], data[2], new_data[0], new_data[1], new_data[2]))
    return result
...
def check_policy_numbers(data):
    ...
    correct = []
    for row in data:
        num20= re.findall(r'20\d{2}\-00\d{4}STR', row[3])
        for num in num20:
            if len(num) > 1:
                correct.append(num)
        numstr = re.findall(r'STR\-000\d{4}', row[3])
        for num in numstr:
            if len(num) > 1:
                correct.append(num)
    result = []
